# Flow 12 Local Messaging Verification Script (PowerShell)
...
$validationResult = go test ./tests -run TestMessageValidation -v
...
$apiResult = go test ./tests -run TestAPIIntegration -v
if ($LASTEXITCODE -eq 0) {
...
$seededResult = go test ./tests -run TestSeededDataVerification -v
if ($LASTEXITCODE -eq 0) {
...
try {
    $messageCount = psql $DATABASE_URL -t -c "SELECT COUNT(*) FROM messages;" 2>$null | ForEach-Object { $_.Trim() }
...
# Check conversations
try {
    $conversationCount = psql $DATABASE_URL -t -c "SELECT COUNT(*) FROM conversations;" 2>$null | ForEach-Object { $_.Trim() }
...
# Get sample message
try {
    $sampleMessage = psql $DATABASE_URL -t -c "SELECT body FROM messages LIMIT 1;" 2>$null | ForEach-Object { $_.Trim() }
} catch {
...
try {
    $directCount = psql $DATABASE_URL -t -c "SELECT COUNT(*) FROM conversations WHERE type = 'direct';" 2>$null | ForEach-Object { $_.Trim() }
    $groupCount = psql $DATABASE_URL -t -c "SELECT COUNT(*) FROM conversations WHERE type = 'group';" 2>$null | ForEach-Object { $_.Trim() }
} catch {
    $directCount = "N/A"
    $groupCount = "N/A"
}
